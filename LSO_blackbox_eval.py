...
from LSO_blackbox_fit import lso_fit, LSOFitNeverGrad, LSOFitBoTorch
...
def evaluate_pmlb_lso(
        ...
        required_columns = ['dataset', 'algorithm', 'random_state', 'training time (s)', 'model_size', 'symbolic_model', 
                            'mse_test', 'mae_test', 'r2_test', 'params_str', 'training time (hr)', 'r2_zero_test',
                            'friedman_dataset', 'symbolic_alg']
        
        params_json = {
                    "reload_model": params.reload_model,
                    "pmlb_data_type": params.pmlb_data_type,
                    "target_noise": params.target_noise,
                    "max_input_points":  params.max_input_points,
                    "lso_optimizer": params.lso_optimizer,
                    "lso_pop_size": params.lso_pop_size,
                    "lso_max_iteration": params.lso_max_iteration,
                    "lso_stop_r2": params.lso_stop_r2,
        ...
        for problem_name in problem_names:
            ...
            while (done_bagging == False) and (bag_number <= max_bags):
                ...
                batch_results["params_str"].extend([params_str])
                ...
                '''
                final_results = batch_results.copy()
                final_results = ensure_columns(final_results, required_columns)
