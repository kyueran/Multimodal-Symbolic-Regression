...
def convert_to_sympy_format(expression_str, features):
    """
    Convert a custom string expression with various operators, constants, variables, and symbols
    into a SymPy-compatible expression, and replace variables with actual features from the dataset.
    """
    # Define the operator maps
    operators_real = {
        "add": "+",
        "sub": "-",
        "mul": "*",
    ...
    # Replace all the operators in the expression
    for op, replacement in operators_real.items():
        expression_str = re.sub(rf'\b{op}\b', replacement, expression_str)

    # Replace variables like "x_0", "x_1", etc., with actual feature names from the dataset
    for i, f in enumerate(features):
        expression_str = expression_str.replace(f'x_{i}', f)

    return expression_str

def complexity_fn(expr):
    c=0
...
def lso_fit(sample_to_learn, env, params, model,batch_results,bag_number, feature_names, true_model):
    ...
    batch_results["process_time"].extend([process_time])
    ...
    try:
        ...
        batch_results["mse_train"].extend([mean_squared_error(y_gt, y)])
        batch_results["mae_train"].extend([mean_absolute_error(y_gt, y)])
        batch_results["r2_train"].extend([r2_score(y_gt, y)])
        ...
        r2_test = r2_score(y_gt_pred, y_pred)
        batch_results["r2_test"].extend([r2_test])

        try:
            converted_expression = convert_to_sympy_format(str(best_eq), feature_names)
            simplified_expr = sp.simplify(converted_expression)

            batch_results["simplified_symbolic_model"].extend([str(simplified_expr)])
            batch_results["simplified_complexity"].extend([complexity_fn(simplified_expr)])
            cleaned_model = simplified_expr
            ...
            if r2_test > 0.5:
                ...
                batch_results['symbolic_error'] = str(sym_diff)
                batch_results['symbolic_fraction'] = str(sym_frac)
                batch_results['symbolic_error_is_zero'] = str(sym_diff) == '0'
                batch_results['symbolic_error_is_constant'] = sym_diff.is_constant()
                batch_results['symbolic_fraction_is_constant'] = sym_frac.is_constant()

        except Exception as e:
            batch_results['sympy_exception'] = str(e)
            if 'symbolic_error_is_zero' not in batch_results.keys():
                batch_results['symbolic_error_is_zero'] = False
            if 'symbolic_error_is_constant' not in batch_results.keys():
                batch_results['symbolic_error_is_constant'] = False
            if 'symbolic_fraction_is_constant' not in batch_results.keys():
                batch_results['symbolic_fraction_is_constant'] = False
